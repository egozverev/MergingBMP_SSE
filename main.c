...
unsigned char *LoadBitmapFile(const char *filename) {
    ...
    if (file_ptr ==
        NULL) { /* if smth went wrong whether it is wrong name or whatever */
        return NULL;
    }
    // read header first
...
void MergeBMPImages(
        const unsigned char *all_data1,
        const unsigned char *all_data2,
        const char *result_filename) {
    // assuming two images in A - {R,B,G} in any order.
    ...
    int32_t byte_string_length =
            header1->width_px * (header1->bits_per_pixel) / 8;
    // num of bytes of each pixel row
    int32_t padding_len =
            byte_string_length % 4 == 0 ? 0 : 4 - byte_string_length % 4;
    for (int32_t current_row = 0; current_row < header1->height_px;
         ++current_row) {
        int32_t current_pixel = 0;
        for (current_pixel = 0; current_pixel < header1->width_px;
             current_pixel += 1) {
            const __m64 *data1_vector_ptr =
                    (__m64 *) (data1 + current_byte); // unsigned ints!
            ...
            float alpha = data2_vector_float[4] / 255;
            ...
            data1_vector_float =
                    _mm_add_ps(data1_vector_float, data2_vector_float);
            data1_vector_float =
                    _mm_round_ps(data1_vector_float, _MM_FROUND_TO_NEAREST_INT);
